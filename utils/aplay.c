...
struct pcm_worker {
	...
	bool transport_volume_update_event;
...
static void set_alsa_playback_volume(struct pcm_worker *w) {
	if ((w->transport.type == BA_PCM_TYPE_A2DP) && (w->melem)) {
		bool ch1_muted, ch2_muted;
		int ch1_volume, ch2_volume;
		double n_vol;
		int delta;

		if (bluealsa_get_transport_volume(w->ba_fd, &w->transport,
						    &ch1_muted, &ch1_volume,
						    &ch2_muted, &ch2_volume) == -1) {
			error("Couldn't get transport volume: %s", strerror(errno));
			return;
		}

		debug("Transport volume: "
		      "ch1_muted=%d, ch1_volume=%d, "
		      "ch2_muted=%d, ch2_volume=%d",
		      ch1_muted, ch1_volume,
		      ch2_muted, ch2_volume);

		n_vol = get_normalized_playback_volume(w->melem, SND_MIXER_SCHN_FRONT_LEFT);
		delta = ch1_volume - lrint(127.*n_vol);
		if (delta)
			set_normalized_playback_volume(w->melem, n_vol + delta/127., delta);
	}
}
...
static void *pcm_worker_routine(void *arg) {
	...
	while (main_loop_on) {
		...
		if (w->transport_volume_update_event) {
			set_alsa_playback_volume(w);
			w->transport_volume_update_event = false;
		}
...
static int melem_event(snd_mixer_elem_t *elem, unsigned int mask) {
	...
	for (i = 0; i < workers_count; i++) {
		...
		if (t->type == BA_PCM_TYPE_A2DP) {
			...
			n_vol = get_normalized_playback_volume(elem, SND_MIXER_SCHN_FRONT_LEFT);
			volume = lrint(127.*n_vol);

			if (volume != t->ch1_volume) {
				debug("Mixer event, setting transport volume to %d", volume);
				bluealsa_set_transport_volume(mpriv->ba_fd, t, 0, volume, 0, volume);
			}
		}
	}
...
usage:
	...
	while (main_loop_on) {
		...
		if (event.mask & BA_EVENT_UPDATE_VOLUME) {
			debug("Transport volume update event");
			for (i = 0; i < workers_count; i++)
				workers[i].transport_volume_update_event = true;
			/* No other events */
			if (event.mask == BA_EVENT_UPDATE_VOLUME)
				continue;
		}
...
init:
		...
		for (i = 0; i < (unsigned)ret; i++) {
			...
			if (!matched) {
				...
				worker->active = false;
				worker->transport_volume_update_event = false;
				worker->pcm_fd = -1;
				worker->ba_fd = -1;
				worker->pcm = NULL;
				worker->melem = melem;
