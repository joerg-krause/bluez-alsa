...
static void *pcm_worker_routine(void *arg) {
	...
	size_t pause_threshold = pcm_1s_samples / snd_pcm_format_size(w->ba_pcm.format, 2);
	...
	if (volume_control)
		w->melem = init_mixer_control(&mixer, device, volume_control);
	...
	while (main_loop_on) {
		...
		/* a mixer event occurs when the volume has been updated */
		if (w->mixer_event) {
			w->mixer_event = false;
			set_transport_volume(w);
		}
		if (w->volume_event) {
			w->volume_event = false;
			set_alsa_playback_volume(w);
		}
