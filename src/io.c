...
#endif
#if ENABLE_APTX || ENABLE_APTX_HD || ENABLE_APTX_DEC
# include <openaptx.h>
#endif
#if ENABLE_MP3LAME
# include <lame/lame.h>
...
static void *io_thread_a2dp_sink_aptx(void *arg) {
	...
	const unsigned int channels = ba_transport_get_channels(t);
	const size_t aptx_frame_size = 4;
	const size_t aptx_pcm_samples = aptx_frame_size * channels;
	...
	if (ffb_init(&pcm, aptx_pcm_samples * aptx_code_len * (mtu_read / aptx_frame_size)) == NULL ||
			ffb_init(&bt, t->mtu_read + sample_size) == NULL) {
		error("Couldn't create data buffers: %s", strerror(ENOMEM));
		goto fail_ffb;
	}
